...
import './header.css'; // Importing the new CSS file
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-3">
            <img
              ...
              onError={(e) =>
                (e.currentTarget.src =
                  "https://placehold.co/80x80?text=Lambang")
              }
            />
          ...
          <nav className="hidden md:flex items-center gap-2">
            ...
            <div className="relative">
              <button
                onClick={() =>
                  setOpenMenu(openMenu === "profil" ? null : "profil")
                }
            ...
            <div className="relative">
              <button
                onClick={() =>
                  setOpenMenu(openMenu === "program" ? null : "program")
                }
                className={`${baseLink} inline-flex items-center gap-1 text-slate-600 opacity-80`}
              >
                Program{" "}
        ...
        {mobileOpen && (
          <div className="md:hidden mt-3 p-3 rounded-2xl border bg-white/70 backdrop-blur">
            ...
            <MobileCollapse
              label="Profil"
              items={menuItemsProfil}
              onNavigate={() => setMobileOpen(false)}
            />
            <MobileCollapse
              label="Program"
              items={menuItemsProgram}
              onNavigate={() => setMobileOpen(false)}
            />
            <MobileLink to="/berita" onClick={() => setMobileOpen(false)}>
              Berita
            </MobileLink>
            <MobileLink to="/pengumuman" onClick={() => setMobileOpen(false)}>
              Pengumuman
            </MobileLink>
            <MobileLink to="/unduhan" onClick={() => setMobileOpen(false)}>
              Download
            </MobileLink>
            <MobileLink to="/pengaduan" onClick={() => setMobileOpen(false)}>
              Pengaduan
            </MobileLink>
          </div>
        )}
